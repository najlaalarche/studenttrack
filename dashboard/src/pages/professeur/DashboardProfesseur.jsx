...
export default function DashboardProfesseur({ professeur, onLogout }) {
  const profModules = professeur?.modules || [];

  // Sélection du module (au niveau du dashboard, pas du login)
  const [selectedModId, setSelectedModId] = useState(
    profModules.length > 0 ? profModules[0].id : null
  );

  const selectedMod  = profModules.find(m => m.id === selectedModId) || null;
  const moduleProp   = selectedMod?.nom    || "";
  const semestreProp = selectedMod?.semestre || "Tous";
  ...
  useEffect(() => {
    if (!moduleProp) { setLoading(false); return; }
    setLoading(true);
    setRows([]);
      ...
      try {
        ...
        for (const etudiant of liste) {
          const modInfo = (etudiant.modules ?? []).find(m =>
            m.module === moduleProp &&
            (!semestreProp || semestreProp === "Tous" || semestreProp === "S0" || m.semestre === semestreProp)
          );
    ...
    load();
  }, [selectedModId, moduleProp, semestreProp]);
  ...
  const modLabel = (m) => {
    let label = m.nom;
    if (m.filiere) label += ` — ${m.filiere}`;
    if (m.semestre && m.semestre !== "S0") label += ` (${m.semestre})`;
    return label;
  };
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#FFFFFF", padding: "40px 32px", maxWidth: 1000, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "#8DC63F", marginBottom: 2 }}>
              Professeur
            </div>
            ...
            <p style={{ fontSize: 13, color: "#64748b", margin: "2px 0 0" }}>
              {profModules.length} module{profModules.length !== 1 ? "s" : ""} enseigné{profModules.length !== 1 ? "s" : ""}
            </p>
          </div>
        </div>
      ...
      {profModules.length === 0 ? (
        <div style={{ padding: "20px 24px", backgroundColor: "#fffbeb", border: "1px solid #fde68a", borderRadius: 8, marginBottom: 24, fontSize: 13, color: "#92400e" }}>
          Aucun module assigné à votre compte. Contactez l'administration.
        </div>
      ) : (
        <div style={{ backgroundColor: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 8, padding: "14px 20px", marginBottom: 24, display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.05em" }}>Module</span>
          <select
            value={selectedModId || ""}
            onChange={e => setSelectedModId(parseInt(e.target.value) || null)}
            style={{ ...selectStyle, minWidth: 280, fontWeight: 600, color: "#1a3a6b" }}
          >
            {profModules.map(m => (
              <option key={m.id} value={m.id}>{modLabel(m)}</option>
            ))}
          </select>
          {selectedMod && (
            <span style={{ fontSize: 12, color: "#64748b" }}>
              {rows.length} étudiant{rows.length !== 1 ? "s" : ""} dans ce module
            </span>
          )}
        </div>
      )}
          ...
          <div style={{ backgroundColor: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 8, padding: "16px 20px", marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginLeft: "auto" }}>
                {hasActiveFilters && (
                  <button onClick={resetFilters} style={{ padding: "5px 12px", borderRadius: 6, fontSize: 12, fontWeight: 500, cursor: "pointer", backgroundColor: "#FFFFFF", color: "#dc2626", border: "1px solid #fca5a5" }}>
                    Réinitialiser
                  </button>
                )}
